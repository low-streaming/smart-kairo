...
const BlockRegistry = {
    renderLight: (b) => {
        ...
        return `<div style="width:100%; height:100%; background:rgba(0,0,0,0.6); backdrop-filter:blur(10px); border:1px solid ${isOn ? color : 'rgba(255,255,255,0.08)'}; border-radius:16px; display:flex; align-items:center; justify-content:center; box-shadow: ${isOn ? '0 0 30px ' + color + '40, inset 0 0 10px ' + color + '20' : 'none'}; transition:0.4s ease;">
            <ha-icon icon="${isOn ? 'mdi:lightbulb-on' : 'mdi:lightbulb-outline'}" style="color:${isOn ? color : '#fff'}; filter:${isOn ? 'drop-shadow(0 0 10px ' + color + ')' : 'none'}; transform:${isOn ? 'scale(1.1)' : 'scale(1)'}; transition:0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275);"></ha-icon>
        </div>`;
    },
    renderFan: (b) => {
        const isOn = b.state === 'on';
        const color = b.color || '#00f6ff';
        return `<div style="width:100%; height:100%; background:rgba(255,255,255,0.02); backdrop-filter:blur(10px); border:1px solid rgba(255,255,255,0.08); border-radius:18px; display:flex; flex-direction:column; align-items:center; justify-content:center; box-shadow:inset 0 0 15px rgba(255,255,255,0.02);">
    ...
    renderSensor: (b) => {
        ...
        return `<div style="width:100%; height:100%; background:rgba(0,0,0,0.4); backdrop-filter:blur(15px); border:1px solid rgba(255,255,255,0.05); border-radius:16px; display:flex; flex-direction:column; align-items:center; justify-content:center; overflow:hidden; position:relative;">
            ...
            <div style="font-size:22px; font-weight:900; color:#fff; text-shadow:0 0 10px rgba(0,0,0,0.5);">${b.state || '24'}<span style="font-size:10px; opacity:0.6; margin-left:2px; font-weight:400;">${b.unit || ''}</span></div>
        </div>`;
    },
    renderClimateArc: (b) => {
        const color = b.color || '#10b981';
        return `<div class="studio-pro-arc" style="background:rgba(0,0,0,0.7); backdrop-filter:blur(20px); border:1px solid ${color}30; box-shadow: 0 15px 50px rgba(0,0,0,0.5), inset 0 0 30px ${color}10; width:100%; height:100%; border-radius:50%; position:relative; overflow:hidden;">
            <div style="position:absolute; inset:5px; border:1px dashed ${color}40; border-radius:50%; opacity:0.5;"></div>
            <div class="val" style="color:#fff; text-shadow:0 0 20px ${color}; font-size:28px; font-weight:900;">${b.text || (b.state + '°')}</div>
        </div>`;
    },
    renderHexPower: (b) => {
        const color = b.color || '#00f6ff';
        const isOn = b.state !== 'off' && b.state !== '0';
        return `<div style="width:100%; height:100%; background:linear-gradient(135deg, ${color}, ${color}80); clip-path:polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%); display:flex; align-items:center; justify-content:center; filter:drop-shadow(0 0 15px ${color}40);">
    ...
    renderPulseChart: (b) => {
        const color = b.color || '#00f6ff';
        return `<div style="width:100%; height:100%; background:rgba(0,0,0,0.6); backdrop-filter:blur(10px); border-radius:18px; border:1px solid rgba(255,255,255,0.06); padding:10px; display:flex; flex-direction:column; box-shadow:inset 0 0 20px rgba(0,0,0,0.4);">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                ...
                <span style="font-size:9px; color:#fff; font-weight:900; background:${color}30; padding:2px 6px; border-radius:10px; border:1px solid ${color}40;">${b.state || '85%'}</span>
            </div>
    ...
    renderWeather: (b) => {
        return `<div style="width:100%; height:100%; background:rgba(255,255,255,0.03); backdrop-filter:blur(10px); border-radius:16px; border:1px solid rgba(255,255,255,0.05); display:flex; align-items:center; gap:10px; padding:0 12px;">
    ...
    renderMediaPlayer: (b) => {
        return `<div style="width:100%; height:100%; background:rgba(0,0,0,0.5); backdrop-filter:blur(15px); border-radius:20px; border:1px solid rgba(255,255,255,0.08); display:flex; align-items:center; gap:10px; padding:10px;">
    ...
    renderSliderDimmer: (b) => {
        ...
        return `<div style="width:100%; height:100%; background:rgba(255,255,255,0.02); backdrop-filter:blur(15px); border-radius:18px; border:1px solid rgba(255,255,255,0.06); display:flex; flex-direction:column; justify-content:center; gap:8px; padding:0 12px; box-shadow:inset 0 0 20px rgba(255,255,255,0.02);">
    ...
    renderEnergyRing: (b) => {
        const color = b.color || '#00f6ff';
        return `<div style="width:100%; height:100%; display:flex; align-items:center; justify-content:center; position:relative; background:rgba(0,0,0,0.4); border-radius:50%; border:1px solid rgba(255,255,255,0.05);">
    ...
    renderModeSwitch: (b) => {
        return `<div style="width:100%; height:100%; background:rgba(0,0,0,0.4); border-radius:12px; display:flex; gap:4px; padding:4px; border:1px solid rgba(255,255,255,0.05);">
    ...
    renderGlassAction: (b) => {
        return `<div style="width:100%; height:100%; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.1); border-radius:12px; backdrop-filter:blur(10px); display:flex; align-items:center; justify-content:center;">
             <ha-icon icon="mdi:gesture-tap" style="color:#fff;"></ha-icon>
        </div>`;
    }
};
...
class OpenKairoCustomCard extends HTMLElement {
  ...
  updateLiveStates() {
    ...
    this._config.layout.forEach(b => {
      ...
      const bWithState = { 
        ...b, 
        state: stateObj ? stateObj.state : (b.state || 'off'), 
        attributes: stateObj ? stateObj.attributes : (b.attributes || {}),
      ...
      try {
        ...
        console.warn("OpenKAIRO Renderer Error:", err, b);
        html = `<div style="color:red; font-size:8px;">Err</div>`;
      }
  ...
  render() { 
    this.innerHTML = `
      <div style="padding:24px; color:#fff; background:rgba(8,8,10,0.9); border-radius:18px; border:1px solid rgba(255,255,255,0.1); font-family:'Outfit', sans-serif;">
         ...
         <div style="font-size:12px; opacity:0.6; line-height:1.6; margin-bottom:15px;">Design-Änderungen und Layouts werden bequem im <b>Kairo Architect</b> (Seitenleiste) vorgenommen. Deine Änderungen werden hier automatisch übernommen.</div>
         
         <div style="background:rgba(255,165,0,0.1); border:1px solid rgba(255,165,0,0.2); border-radius:12px; padding:12px; margin-bottom:20px;">
            <div style="font-size:10px; font-weight:900; color:#ffa500; text-transform:uppercase; margin-bottom:4px;">PRODUKTIONS-MODUS</div>
            <div style="font-size:10px; opacity:0.8; line-height:1.4;">Der Architect ist aktuell in der Seitenleiste ausgeblendet. Um ihn zu nutzen, muss er in der <code>__init__.py</code> (Developer Mode) wieder einkommentiert werden.</div>
         </div>

         <button style="width:100%; padding:12px; background:#00f6ff; border:none; border-radius:10px; font-weight:900; cursor:pointer;" onclick="window.top.location.href='/openkairo'">ARCHITECT ÖFFNEN</button>
      </div>`; 
  }
}
